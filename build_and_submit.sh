...
path_to_spark="$HOME/hadoop/spark-3.0.1-bin-hadoop2.7"

if [ -n "$2" ]; then path_to_spark=$2; fi

$path_to_spark/bin/spark-submit --conf spark.eventLog.enabled=true --conf spark.eventLog.dir="/tmp" --driver-memory 10g --class paristech.$1 target/scala-2.12/*.jar
